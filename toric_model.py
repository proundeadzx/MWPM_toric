...
class Toric_code():
    ...
    def generate_random_error(self, p_error, pre_error_coords = []):
        for i in range(2):
            qubits = np.random.uniform(0, 1, size=(self.system_size, self.system_size))
            if (len(pre_error_coords) > 0):
                qubits[pre_error_coords[i]]= 0
                self.qubit_matrix[i, pre_error_coords[i]]= 8
        ...
        self.syndrom('state')
        #self.plot_toric_code(self.state, 'init')
    ...
    def generate_n_plus_q_random_errors(self, q, p_error):
        self.generate_n_random_errors(q)
        error_coords0 = np.nonzero(self.qubit_matrix[0,:,:])
        error_coords1 = np.nonzero(self.qubit_matrix[1,:,:])
        error_coords = [error_coords0, error_coords1]
        self.generate_random_error(p_error, error_coords)
